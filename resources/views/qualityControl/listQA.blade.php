...
<div class="mdk-header-layout__content page">   
    <div class="page__heading">
        ...
        <h1 class="m-0">Boletas para el Control de Calidad</h1>
    </div>
</div>
        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                <div class="form-group text-right">
                    <a href="{{route('qualityControl.registerQA')}}" class="btn btn-success" style="margin-top: 25px;">
                        Nuevo Control de Calidad <i class="material-icons">add</i></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="card">
    <div class="table-responsive">
        <table id="productQATable" class="table mb-0 thead-border-top-0 table-striped">
            <thead>
                <tr>
                    <th class="text-center">
                        <i class="material-icons icon-16pt mr-1">map</i><span>No.Revisión</span>
                    </th>
                    <th class="text-center">
            ...
            <tbody class="list" id="companies">
                @foreach($QAControls as $QAControl)
                <tr>
                    <td class="text-center">
                        <div class="badge badge-light">#{{$QAControl -> idQualityControl}}</div>
                    </td>
                    <td class="text-center">
                        <a href="{{route('billingOrders.viewBillingOrder', ['id' => $QAControl -> productSale -> billingOrders -> idbillingOrder])}}">
                            #{{$QAControl -> productSale -> billingOrders -> idbillingOrder}}</a>
                    </td>
                    <td class="text-center">
                        <a href="{{route('customers.viewCustomer', ['id' => $QAControl -> productSale -> billingOrders -> customers -> idCustomer])}}">
                            {{$QAControl -> productSale -> billingOrders -> customers -> customerFullName}}</a>
                    </td>
                    <td class="text-center"><strong>{{$QAControl -> productSale -> products -> productName}}<strong></td>
                    <td class="text-center">{{$QAControl -> technicianAssigned}}</td>
                    <td class="text-center">{{$QAControl -> dateCreation}}</td>
                    <div class="dropdown ml-auto">
                        <td class="text-center" style="width: 81px;">
                            <a href="{{route('qualityControl.viewQA', ['id' => $QAControl -> idQualityControl])}}" data-caret="false" class="text-muted" title="Ver Orden de Facturación">
                                <i class="material-icons">visibility</i></a>
                            <a href="{{route('qualityControl.editQA', ['id' => $QAControl -> idQualityControl])}}" data-caret="false" class="text-muted" title="Editar Orden de Facturación">
                                <i class="material-icons">edit</i></a>
                            <a href="{{route('qualityControl.deleteQualityControl', ['id' => $QAControl -> idQualityControl])}}" data-id="{{$QAControl -> idQualityControl}}" data-caret="false" 
...
@endif 
<script src="{{asset('HTML/dist/assets/js/listProductQA.js')}}"></script>
@endsection