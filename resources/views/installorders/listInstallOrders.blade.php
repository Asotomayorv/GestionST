...
<div class="card card-form d-flex flex-column flex-sm-row">
    <div class="card-form__body card-body-form-group flex">
        <div class="row">
            ...
            <div class="form-group">
                <label for="filter_stock">Estado Instalación</label>
            ...
            <div class="col-sm-4">
                <div class="form-group text-right">
                    <a href="{{route('installorders.createInstallOrder')}}" class="btn btn-success" style="margin-top: 25px;">
                        Nueva Orden de Instalación<i class="material-icons">add</i></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="card">
    <div class="table-responsive">
        <table id="installTable" class="table mb-0 thead-border-top-0 table-striped">
            <thead>
                <tr>
            ...
            <tbody class="list" id="companies">
                @foreach($installorders as $install)
                <tr>
                    <td class="text-center">
                        <div class="badge badge-light">#{{$install -> idinstallation}}</div>
                    </td>
                    <td class="text-center">{{$install -> idRoute}}</td>
                    <td class="text-center">
                        <a href="{{route('customers.viewCustomer', ['id' => $install -> routes -> customers -> idCustomer])}}">
                            {{$install -> routes -> customers -> customerFullName}}</a>
                    </td>
                    <td class="text-center"><strong>{{$install -> productInstall -> first() -> products -> productName}}<strong></td>
                    <td class="text-center">{{$install -> routes -> routeTechnicianAssigned}}</td>
                    <td class="text-center">{{$install -> installationDate}}</td>
                    <td class="text-center">
                        @if($install->installationStatus == 0)
                            <div class="badge badge-warning">PENDIENTE</div>
                        @elseif($install->installationStatus == 1)
                    ...
                    <div class="dropdown ml-auto">
                        <td class="text-center" style="width: 81px;">
                            <a href="{{route('installorders.viewInstallOrder', ['id' => $install -> idinstallation])}}" data-caret="false" class="text-muted" title="Ver Orden de Facturación">
                                <i class="material-icons">visibility</i></a>
                            <a href="{{route('installorders.editInstallOrder', ['id' => $install -> idinstallation])}}" data-caret="false" class="text-muted" title="Editar Orden de Facturación">
                                <i class="material-icons">edit</i></a>
                            <a href="{{route('installorders.deleteInstallOrder', ['id' => $install -> idinstallation])}}" data-id="{{$install -> idinstallation}}" data-caret="false" 
...
@endif 
<script src="{{asset('HTML/dist/assets/js/listInstallOrder.js')}}"></script>
@endsection