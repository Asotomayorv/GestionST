...
$(document).ready(function() {
    ...
    $("a[data-id]").click(function(e) {
        e.preventDefault();
        var billingOrderId = $(this).data('id');
        ...
        $('#confirmDeleteButton').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    toastr.success('Boleta de Faturación eliminada exitosamente.');
                    location.reload(); // Recargar la página 
                },
                error: function(xhr, status, error) {
                    // Manejar errores si es necesario
                    toastr.warning('Existen garantías o controles de calidad para esta boleta de facturación');
                }
            });
        });
    });
    ...
    // Filtro por Cédula
    $('#filter_invoiceNumer').on('keyup', function() {
        var invoiceNumber = $(this).val().trim();
        table.column(0).search(invoiceNumber).draw();
    });
    ...
    $('#filter_seller').on('change', function() {
        ...
        if (selectedSeller === 'Todos') {
            ...
            table.column(3).search(selectedSeller).draw();
        }
    });
