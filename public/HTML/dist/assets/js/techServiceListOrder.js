...
$(document).ready(function() {
    ...
    table = $('#techServiceTable').DataTable({
        language: {
    ...
    $("a[data-id]").click(function(e) {
        e.preventDefault();
        var techServiceOrderId = $(this).data('id');
        ...
        $('#confirmDeleteButton').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    toastr.success('La Boleta de Servicio Técnico se eliminó existosamente.');
                    location.reload(); // Recargar la página 
                },
                error: function(xhr, status, error) {
                    // Manejar errores si es necesario
                    toastr.warning('Ocurrió un error al intentar eliminar la Boleta de Servicio Técnico: ' + error);
                }
            });
        });
    });
    ...
    $('#inlineFormStatus').on('change', function() {
        ...
        table.column(5).search(isChecked ? 'FINALIZADO' : '').draw();
        //Mmensaje indicando el filtro actual
        if (isChecked) {
            toastr.info('Mostrando boletas de Servicio Técnico finalizadas.');
        } else {
            toastr.info('Mostrando todas las boletas de Servicio Técnico.');
        }
    });
    ...
    $('#filter_technician').on('change', function() {
        ...
        if (selectedTechnician === 'Todos') {
            ...
            table.column(3).search(selectedTechnician).draw();
        }
    });
