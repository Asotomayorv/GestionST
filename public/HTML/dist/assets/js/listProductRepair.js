...
$(document).ready(function() {
    ...
    table = $('#productRepairTable').DataTable({
        language: {
    ...
    $("a[data-id]").click(function(e) {
        e.preventDefault();
        var repairDetailId = $(this).data('id');
        ...
        $('#confirmDeleteButton').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    toastr.success('El trabajo de reparación se eliminó existosamente.');
                    location.reload(); // Recargar la página 
                },
                error: function(xhr, status, error) {
                    // Manejar errores si es necesario
                    toastr.warning('Ha ocurrido un error al intentar eliminar el trabajo de reparación: ' + error);
                }
            });
        });
    });
    ...
    $('#filter_name').on('keyup', function() {
        ...
        table.column(1).search(customerName).draw();
    });

    // Filtro por Nombre
    $('#filter_id').on('keyup', function() {
        var billingOrder = $(this).val().trim();
        table.column(0).search(billingOrder).draw();
    });
