$(document).ready(function() {
    ...
    function validateCheckboxes() {
        // Array de nombres de las casillas de verificación
        var checkboxNames = ['accesories', 'adapter', 'display', 'keyboard', 'cases', 'carReader', 'battery', 'sound', 'wifi',
            'installationBase', 'communication', 'camera', 'tcpIp', 'downUSB', 'rs485', 'downSDCard', 'miniUSB', 'rs232', 'reader'
        ];
        // Verificar si al menos una casilla está marcada
        var atLeastOneChecked = checkboxNames.some(function(name) {
            return $('#' + name).prop('checked');
        });
        // Mostrar el mensaje de error si ninguna casilla está marcada
        if (!atLeastOneChecked) {
            $("#checkDetails").addClass("is-invalid");
            $('#checkDetails-error').show();
            return false;
        } else {
            // Si al menos una casilla está marcada, oculta el mensaje de error y envía el formulario
            $("#checkDetails").removeClass("is-invalid");
            $('#checkDetails-error').hide();
            return true;
        }
    }

    // Manejador de envío del formulario
    $("#registerQualityControl").on("submit", function(event) {
        if (!validateProduct() || !validateCheckboxes() || !validateTechnicianComments()) {
    ...
    // Manejador de envío del formulario
    $("#modifyQualityControl").on("submit", function(event) {
        if (!validateCheckboxes() || !validateTechnicianComments()) {
            event.preventDefault(); // Evita el envío del formulario
        }
    });
});