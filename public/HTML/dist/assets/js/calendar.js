$(document).ready(function() {
    ...
    $('#routeCalendar').fullCalendar({
        ...
        eventClick: function(event, jsEvent, view) {
            ...
            $('#modifyRouteForm #modifyRouteAddress').val(selectedEvent.routeAddress);
            ...
            $('#modifyRouteForm #modifyRouteComments').val(selectedEvent.routeComments);
        ...
        eventRender: function(event, element, view) {
            ...
            var eventDetails = '<strong>' + event.customer + '</strong><br>';
    ...
    function loadDistritosOfSelectedCanton() {
        var selectedProvince = $('#province').val(); // Obtener el valor de la provincia seleccionada
        var selectedCanton = $('#canton').val(); // Obtener el valor del cantón seleccionado
        var url = "https://ubicaciones.paginasweb.cr/provincia/" + selectedProvince + "/canton/" + selectedCanton + "/distritos.json"; // Construir la URL para obtener los distritos
    ...
    // Manejador de entrada para la dirección
    $("#modifyRouteAddress").on("blur", function() {
        validateModifyRouteAddress();
    });

    // Manejador de entrada para la dirección
    $("#modifyRouteComments").on("blur", function() {
        validateModifyRouteComments();
    });
    ...
    // Validación de la dirección
    function validateModifyRouteAddress() {
        var modifyRouteAddress = $("#modifyRouteAddress").val();
        if (modifyRouteAddress.trim() === "") {
            $("#modifyRouteAddress").addClass("is-invalid");
            $("#modifyRouteAddress-error").show();
            return false;
        } else {
            $("#modifyRouteAddress").removeClass("is-invalid");
            $("#modifyRouteAddress-error").hide();
            return true;
        }
    }
    ...
    // Validación de la dirección
    function validateModifyRouteComments() {
        var modifyRouteComments = $("#modifyRouteComments").val();
        if (modifyRouteComments.trim() === "") {
            $("#modifyRouteComments").addClass("is-invalid");
            $("#modifyRouteComments-error").show();
            return false;
        } else {
            $("#modifyRouteComments").removeClass("is-invalid");
            $("#modifyRouteComments-error").hide();
            return true;
        }
    }
    ...
    $("#createRouteForm").on("submit", function(event) {
        if (!validateCreateCustomerID() || !validateRouteAddress() || !validateRouteComments()) {
            event.preventDefault(); // Evita el envío del formulario
        }
    });

    // Manejador de envío del formulario
    $("#modifyRouteForm").on("submit", function(event) {
        if (!validateModifyRouteAddress() || !validateModifyRouteComments()) {
            event.preventDefault(); // Evita el envío del formulario
        }
    });
