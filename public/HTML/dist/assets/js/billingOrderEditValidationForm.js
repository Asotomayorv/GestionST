$(document).ready(function() {
    ...
    function populateSelectedProductsTable(selectedProducts) {
        ...
        selectedProducts.forEach(function(product) {
            ...
            var newRow = '<tr>' +
                ...
                '<td class="text-center">' + productDetails.productQuantity + '</td>' +
                ...
                '<td class="text-center">' +
                '<div class="custom-control custom-checkbox">' +
                '<input type="checkbox" class="custom-control-input" id="checkbox' + product.idproduct + '" name="installationRequired" value="' + product.installationRequired + '" ' + (product.installationRequired ? 'checked' : '') + '>' +
            ...
            // Agrega el evento blur para capturar el valor después de que el usuario sale del campo
            selectedProductTable.off('blur').on('change', '.product-quantity', function() {
                var productQuantityValueSelected = $(this).val();
                console.log("productQuantity value:", productQuantityValueSelected);
                if (productQuantityValueSelected < 1) {
                    toastr.warning('La cantidad del producto no puede ser 0');
                }
                // Verifica si la cantidad seleccionada es mayor a la cantidad de productos disponibles
                if (productQuantityValueSelected > parseInt(productDetails.productQuantity)) {
                    toastr.warning('La cantidad seleccionada no puede ser mayor a la cantidad de productos disponibles.');
                }
                // Actualiza los datos en el campo oculto
                updateSelectedProductData();
    ...
    /* Agrega el evento change para el input de cantidad
    $('#selectedProductTable tbody').on('change', '.product-quantity', function() {
        var productQuantityValueSelected = $(this).val();
        console.log("productQuantity value:", productQuantityValueSelected);
        updateSelectedProductData();
    }); */
    ...
    $("#modifyBillingOrder").on("submit", function(event) {
        ...
        if (rowCount === 0) {
            toastr.warning('Los productos a facturar no puede quedar vacía, por favor verifica e intenta de nuevo.');
            event.preventDefault(); // Evita el envío del formulario
            return;
        }
